...
class Parser:
    ...
    def parse_attribute(self, name: Name) -> Attribute:
        ...
        if type(token) is Token and token.kind is TokenKind.Dot:
            ...
            if type(next_name) is not Name:
                raise TypeError(f"expecting a Name asfter {name}, got {next_name}. at line {name.line}. in module {self.module_name}")
    ...
    def parse_subscript(self, name: Name):
        ...
        if not (type(token) is Token and token.kind is TokenKind.LeftBracket):
            raise SyntaxError(f"expected '[', got {token.format}. at line {token.line}. in module {self.module_name}")
    ...
    def parse_type_name(self, name: Name | Attribute | Subscript):
        if not (type(name) is Name or type(name) is Attribute or type(name) is Subscript):
            raise TypeError(f'name must be Name, Attribute or Subscript, found Token "{name.format}". in line {name.line}. at module {self.module_name}')
        ...
        if type(expression) is Token:
            if expression.kind in TOKENKIND_UNARYOPERATORS:
                ...
                if type(right_parenthesis) is Token and right_parenthesis.kind is TokenKind.RightParenthesis:
                    raise SyntaxError(f"Empty parenthesized expression, at line {expression.line}. in module {self.module_name}")
                else:
                    self.source.release()
            ...
            raise TypeError(f"expression must be Ast, found Token '{expression.format}'. in line {expression.line}. at module {self.module_name}")
        elif type(expression) is Keyword:
            if expression.kind is KeywordKind.Comptime:
                return self.parse_comptime()

            raise TypeError(f'expression must be Ast, found Keyword "{expression.format}". in line {expression.line}. at module {self.module_name}')
    ...
    def parse_function_head(self) -> FunctionHead:
        ...
        if not (type(colon) is Token and colon.kind is TokenKind.Colon):
            raise SyntaxError(f"expecting ':' before parameter type of {colon}. in module {self.module_name}")
    ...
    def parse_extern(self) -> Extern:
        ...
        raise SyntaxError(f'missing keyword fun. at line {token.kind}. in module {self.module_name}')
    ...
    def parse_variable_declaration(self) -> VariableDeclaration:
        ...
        if not (type(equal) is Token and equal.kind is TokenKind.Equal):
            raise SyntaxError(f"expecting '=' before parameter type of let {name.value}. at line {equal.line}. in {self.module_name}")
